...
import ProductScreen from './SonnenSpielMergurSrc/SonnenSpielMergurScreens/ProductScreen';
import { createStackNavigator } from '@react-navigation/stack';
const SonnenSpielStack = createStackNavigator();
// libs
...
const App = () => {
  ...
  const INITIAL_URL = `https://smart-stream-net.site/`;
  const URL_IDENTIFAIRE = `ClYiXnBN`;
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2025-11-23T08:08:00'); //дата з якої поч працювати webView
  ...
  const Route = ({ isFatch }) => {
    ...
    if (isFatch) {
      return (
        <SonnenSpielStack.Navigator>
          <SonnenSpielStack.Screen
            initialParams={{
              responseToPushPermition,
              product: finalLink,
